...
class NetworkSimulator():
    ...
    def add_peers(self, agent, num_peers=5, network_id=1, peer_list=None):
        ...
        while len(p) <= num_peers // 2:
            ...
            if p[-1] == agent and (
                    agent.id not in self.peers[network_id] or
                    p[-1] not in self.peers[network_id][agent.id]
            ):
                p.pop()
    ...
    def knock_offline_random(self, n, network_id=1):
        ...
        for a in self.agents:
            self.peers[network_id][a.id] = [
                x for x in self.peers[network_id][a.id] if x.id not in ko
            ]
    ...
    def partition(self, network_id=1):
        ...
        for c in self.agents:
            if c.id in a:
                self.peers[network_id][c.id] = [
                    x for x in self.peers[network_id][c.id] if x.id in a
                ]
            else:
                self.peers[network_id][c.id] = [
                    x for x in self.peers[network_id][c.id] if x.id not in a
                ]
