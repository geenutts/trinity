...
from tests.components.eth2.validator_helpers import (
    get_linked_validators,
    get_slot_with_validator_selected,
    get_validator,
)
...
async def test_validator_propose_block_succeeds(event_loop, event_bus):
    ...
    slot, proposer_index = get_slot_with_validator_selected(
...
async def test_validator_handle_first_tick(event_loop, event_bus, monkeypatch):
    ...
    slot_to_propose, index = get_slot_with_validator_selected(
        alice.validator_privkeys,
        state,
        state_machine.config,
    )
